...
<html lang="de">
  ...
  <body>
    ...
    {% else %}
      ...
      <div>
        {% for num in tuerchen %}
          <a href="{% if num <= heute.day and not tuerchen_status[num] %}/oeffne_tuerchen/{{ num }}{% else %}#{% endif %}" class="tuerchen{% if num < heute.day %} disabled{% endif %}" style="background-color: {{ tuerchen_farben[num-1] }}">
            {{ num }}
          </a>
        {% endfor %}
      </div>
